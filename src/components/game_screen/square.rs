...
pub(crate) fn Square(tile: Tile) -> Element {
    ...
    let piece = game_ctrl.game_copy.read().state.board.get_piece(tile);
    let mut classes = vec!["square"];
    let special_tiles = game_ctrl.game_copy.read().logic.board_geo.special_tiles;
    let rules = game_ctrl.game_copy.read().logic.rules;
